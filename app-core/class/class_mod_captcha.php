...
class TextClass {
	...
	public function phptext($text,$textColor,$backgroundColor='',$fontSize,$imgWidth,$imgHeight,$dir,$fileName) {
		/* settings */
		$font = DIRPATH . '/font/calibri.ttf'; // define font
	...
	public function phpcaptcha($textColor,$backgroundColor,$imgWidth,$imgHeight,$noiceLines=0,$noiceDots=0,$noiceColor='#162453') {	
		/* Settings */
		$text=$this->random();
		$font = DIRPATH . '/font/monofont.ttf'; // font
		$textColor=$this->hexToRGB($textColor);	
		...
		if(isset($_SESSION)) {
			session_start(); // Open session file
			$_SESSION['SECURITY_CODE'] = $text; // set random text in session for captcha validation
