...
$page_title = "Aadhaar Card Photo Verification";
$page_link = "";
                    ...
                    <div class="card card-outline card-brand">
                        <div class="card-body min-high2">
                            <div class="row justify-content-center">
                                <form id="app-service-aadhar-cam" name="app-service-aadhar-cam" class="form-material">
                                    ...
                                    <div class="row justify-content-center">
                                        ...
                                        <div class="row mt-3">
                                            <div class="col-md-12 text-center h5">Take your Aadhaar Card <span id="Labelimg">Front</span> photo</div>
                                        </div>
                                    ...
                                    <div class="row mt-4">                    
                                        <div class="col-md-12 mt-3 text-center">
                                            <?php echo '<button type="button" class="btn btn-secondary px-3 border"  name="prevBtn" id="prevBtn" onclick=gobackbut("'.$main_app->strsafe_input($enc_assref_num).'");> Go Back</button>';?>                              
                                             <button type="button" class="btn btn-primary px-4 mr-2 border" name="nextBtn" id="sbt-cust-img-front"  style="display:none;" onclick="upload_livePhoto('WEBCAM_IMAGE','FRONT');">Next <i class="mdi mdi-arrow-right"></i></button>    
                                             <button type="button" class="btn btn-primary px-4 mr-2 border" name="nextBtn" id="sbt-cust-img-bck"  style="display:none;" onclick="upload_livePhoto('WEBCAM_IMAGE','BACK');">Next <i class="mdi mdi-arrow-right"></i></button>    
...
<script type="text/javascript">
    ...
    function gobackbut(ass_ref_num) {
        goto_url('ass-form-aadhaarview?ref_Num='+ass_ref_num);
    }


    function try_Photo(){
       
        labelimg = $('#Labelimg').html();
        $('#WEBCAM_IMAGE').val('');
        setup(); 
        show('my_camera');
        hide('results');
        ...
        hide('try_caption');
        if(labelimg == 'Front'){
            hide('sbt-cust-img-front');
        }else{
            hide('sbt-cust-img-bck');
        }
    ...
    function capturePhoto() {
        ...
        labelimg = $('#Labelimg').html();
        if(labelimg == 'Front'){
            show('sbt-cust-img-front');
        }else{
            show('sbt-cust-img-bck');
        }
    ...
    function upload_livePhoto(file_id,file_mode) {
        if($('#WEBCAM_IMAGE').val() == "") {
			swal.fire('','Snap Photo is required');
		} else {
            loader_start();
            disable('nextBtn');
            ...
            if(file_mode == 'FRONT') {
                var filename = 'AADHAAR1F.jpg';
            } else {
                var filename = 'AADHAAR2B.jpg';
            }
           
            var formData = new FormData();
            formData.append('cmd','ass_aadhaarcard_camera');
            formData.append('asnVal',$('#asnVal').val()); //ref no
            formData.append('FILE_MODE',file_mode); //mode- live photo , gpc
            formData.append('UPLOAD_FILE',blob, filename); //file data
            formData.append('token', '<?php echo (isset($_SESSION['APP_TOKEN'])) ? $_SESSION['APP_TOKEN'] : ""; ?>');
