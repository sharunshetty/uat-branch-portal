...
$page_title = "Branch User Report";
$page_link = "./branch-user-report";
...
$add_sql = "{$primary_key} IS NOT NULL";// AND CR_BY ='".$_SESSION['USER_ID']."'
...
if(isset($_GET['status']))  { $status = $main_app->strsafe_input(trim($_GET['status'])); $filter = true; $additionalVars .= "&status=".$status; } else { $status = ""; }
  ...
  // Custom Search box
    ...
    if(isset($status) && $status != ""){
        if($status == "AP") {//pending for approval
            $add_sql .= " AND TRIM(APP_STATUS) = :APP_STATUS";
            $add_data['APP_STATUS'] = "$status";    
        }elseif($status == "P") {//user pending
            $add_sql .= " AND APP_STATUS IS NULL";           
        }else{// authoriser approval
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-2 col-md-4 form-group">
                                    ...
                                    <select name="status" id="status" class="form-control border-input" autocomplete="off">
                                        ...
                                        <option value="P"> User Pending</option>
                                        <option value="AP">Approval Pending</option>
                                        <option value="AS">Completed </option>
                                        <option value="AR">Rejected</option>
                                    </select>                                  
                                </div>

							
				                <div class="col-lg-2 col-md-4 form-group d-flex align-items-end">
                                    ...
                                    if($totalResults) {
                                        ...
                                        while ($row = $sql_exe->fetch()) {
                                            ...
                                            echo "<td class='text-center'>";  
                                                if(trim($row['APP_STATUS']) == "AP") {
                                                    
                                                    echo "<span class='badge badge-primary'> Approval Pending </span>";
                                                    
                                                }elseif(trim($row['APP_STATUS']) == "S") {
                                                      if(trim($row['AUTH_STATUS']) == "AS") {
                                                    ...
                                                    } elseif(trim($row['AUTH_STATUS']) == "AR") {
                                                    ...
                                                    echo "<span class='badge badge-warning'> User Pending </span>";                                                   
                                                }
                                            ...
                                            echo "<td>";
                                                ...
                                                echo "<a href='javascript:void(0);' class='btn btn-danger btn-block btn-sm p-1' data-value-1='{$safe->str_encrypt($row['ASSREQ_REF_NUM'],$_SESSION['SAFE_KEY'])}' onclick=fetch_modal('customer_review_request','xl','',$(this).data('value-1'));> View <i class='mdi mdi-eye-outline'></i></a>";
