...
if(check_usr_login() == false) {
    echo "<script> sess_error('Session expired. Please login again.'); </script>";
    exit();
}
...
$cmd = str_replace('@','/dy.',$cmd); // Access Folder

// Check for Sub-File
if(file_exists(dirname(__FILE__).'/dy.'.$cmd.'.php')) {
    require_once(dirname(__FILE__).'/dy.'.$cmd.'.php'); //Success
} else {
