    ...
    if($_POST['field_name'] == "modify" && $_POST['field_val'] != "") {
        ...
        if(isset($item_data['USER_ID'])) {
           // $html_op .= "$('#OPERATION').val(decode_ajax('".$main_app->strsafe_ajax('M')."')); $('#STATUS').val(decode_ajax('".$main_app->strsafe_ajax($item_data['USER_STATUS'])."')); focus('STATUS');";
            ...
            $html_op .= "$('#USER_BRANCH').val(decode_ajax('".$main_app->strsafe_ajax($item_data['USER_REGIONS'])."'));";
           
            // $html_op .= "$('#USER_EMAIL').val(decode_ajax('".$main_app->strsafe_ajax($item_data['USER_EMAIL'])."'));";
            $html_op .= "$('#STATUS').val(decode_ajax('".$main_app->strsafe_ajax($item_data['USER_STATUS'])."'));";
        }
        // else {
