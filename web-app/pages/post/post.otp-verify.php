...
if( isset($_POST['LOGIN_OTP']) ) {
    $_POST['LOGIN_OTP'] = preg_replace('/\s+/', '', $_POST['LOGIN_OTP']);
}
...
if($_SESSION['USER_LOGIN_ID'] != "" && $_POST['LOGIN_OTP'] != "") {
    $sql_exe = $main_app->sql_run("SELECT * FROM $page_table_name WHERE LOGIN_REQ_ID = :LOGIN_REQ_ID AND LOGIN_OTP_STATUS = '2'", array('LOGIN_REQ_ID' => $_SESSION['USER_LOGIN_ID']));
...
elseif(!isset($_POST['LOGIN_OTP']) || $_POST['LOGIN_OTP'] == NULL) {
...
elseif(isset($_SESSION['LOGIN_OTP_CHECK_COUNT']) && $_SESSION['LOGIN_OTP_CHECK_COUNT'] > "3") {
...
elseif($_POST['LOGIN_OTP'] != $item_data['OTP_CODE']) {
    echo "<script> swal.fire('','OTP entered is incorrect'); loader_stop(); enable('sbt'); </script>";
    session_start();
    $_SESSION['LOGIN_OTP_CHECK_COUNT'] = (isset($_SESSION['LOGIN_OTP_CHECK_COUNT']) && $_SESSION['LOGIN_OTP_CHECK_COUNT'] != NULL) ? $_SESSION['LOGIN_OTP_CHECK_COUNT'] + "1" : "1";
    session_write_close();
}
else {
    ...
    if($updated_flag == true) {
        ...
        $go_url = APP_URL ."/pages/otp-email-verify"; // Page Refresh URL
