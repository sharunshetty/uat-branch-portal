...
if($page_primary_keys['USER_ID'] !='' && $page_primary_keys['USER_ID'] != NULL) {
    ...
    // $sql_exe = $main_app->sql_run("SELECT * FROM {$page_table_name} WHERE USER_ID = :USER_ID and USER_STATUS != 'A'", $page_primary_keys);
    // $item_data = $sql_exe->fetch();
...
if($page_primary_keys['USER_ID'] != NULL && $item_data2['USER_ID'] !='' && $item_data2['USER_ID'] != NULL) {
    ...
    if(!isset($_POST['TRANSFER_FROMBRANCH'])  || $_POST['TRANSFER_FROMBRANCH'] == NULL) {
        echo "<script> focus('TRANSFER_FROMBRANCH'); swal.fire('','Transfer From Bank is Blanked'); loader_stop(); enable('sbt'); </script>";
        exit();
    }
    elseif(!isset($_POST['TRANSFER_TOBRANCH']) || $_POST['TRANSFER_TOBRANCH'] == NULL){
        echo "<script> focus('TRANSFER_TOBRANCH'); swal.fire('','Please select Transfer To Bank'); loader_stop(); enable('sbt'); </script>";
        exit();
    }
    elseif(!isset($_POST['TRANSFER_DATE']) || $_POST['TRANSFER_DATE'] == NULL) {
        echo "<script> focus('TRANSFER_DATE'); swal.fire('','Please select Transfer Date'); loader_stop(); enable('sbt'); </script>";
        exit();
    }
    elseif($_POST['TRANSFER_FROMBRANCH'] == $_POST['TRANSFER_TOBRANCH']) {
        echo "<script> focus('TRANSFER_TOBRANCH'); swal.fire('','Same Bank Branch Cannot be Transfer'); loader_stop(); enable('sbt'); </script>";
        exit();
    }else{
        ...
        if(isset($cust_count) && $cust_count > "0") {
            echo "<script> swal.fire('','Account opening for customers is still pending, cannot transfer user'); loader_stop(); enable('sbt'); </script>";
            exit();
        }
        ...
        if($auth_ref_num) {
            ...
            $data['TRANSFER_FROMBRANCH'] = $_POST['TRANSFER_FROMBRANCH'];
            $data['TRANSFER_TOBRANCH'] = $_POST['TRANSFER_TOBRANCH'];
            $data['TRANSFER_DATE'] = date("Y-m-d", strtotime($_POST['TRANSFER_DATE']));
            $data['USER_REGIONS'] = $_POST['TRANSFER_FROMBRANCH'];
            ...
            if($updated_flag = true){
                ...
                // $TRANSFER_DATE = new DateTime($_POST['TRANSFER_DATE']); 
                // $CUR_DATE = new DateTime(); 

                // if($TRANSFER_DATE <= $CUR_DATE) {
                //    // $data1['USER_STATUS'] = $_POST['STATUS'];
                //    $data1['TRANSFER_DATE'] = (isset($_POST['TRANSFER_DATE']) && $_POST['TRANSFER_DATE'] !='') ? date("Y-m-d", strtotime($_POST['TRANSFER_DATE'])) : "";
                // }

                //$data1['USER_REGIONS'] = $item_data['TRANSFER_TOBRANCH']; //if transfer branch 
