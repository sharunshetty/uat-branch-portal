...
elseif(!empty($_FILES['UPLOAD_FILE']['name']) && $dataSize > $allowed_size) {
    ...
    //Max. Sl.
    $doc_sl = $main_app->sql_fetchcolumn("SELECT NVL(MAX(DOC_SL), 0) + 1 FROM ASSREQ_CUST_DATA WHERE ASSREQ_REF_NUM = :ASSREQ_REF_NUM AND DOC_CODE = 'CUSTSIGNIMG'", array("ASSREQ_REF_NUM" => $item_data['ASSREQ_REF_NUM'])); // Seq. No.
    ...
    $db_output = $main_app->sql_insert_data("ASSREQ_CUST_DATA", $data, array('ASSREQ_REF_NUM' => $item_data['ASSREQ_REF_NUM'] )); // Update
    if($db_output == false) { $updated_flag = false; }
    
   if($updated_flag == true) {
        $data1 = array();
        $data1['ASSREQ_CUSTSIGN_FLAG'] = "Y";
    ...
    if($updated_flag == true) {  
        ...
        $go_url = "form-branch-details?ref_Num=".$sid_ref_num; // Page Refresh URL
