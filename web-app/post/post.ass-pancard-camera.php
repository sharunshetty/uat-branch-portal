...
elseif(!empty($_FILES['UPLOAD_FILE']['name']) && $dataSize > $allowed_size) {
    ...
    //Max. Sl.
    $doc_sl = $main_app->sql_fetchcolumn("SELECT NVL(MAX(DOC_SL), 0) + 1 FROM ASSREQ_CUST_DATA WHERE ASSREQ_REF_NUM = :ASSREQ_REF_NUM AND DOC_CODE = 'PANIMG'", array("ASSREQ_REF_NUM" => $item_data['ASSREQ_REF_NUM'])); // Seq. No.
    ...
    if($updated_flag == true) {  
        ...
        $go_url = "ass-form-camera?ref_Num=".$sid_ref_num; // Page Refresh URL
