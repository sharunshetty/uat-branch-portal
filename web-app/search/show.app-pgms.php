...
/** Modal Title */
$ModalLabel = "Search : Programs List";
...
<div class="modal-body py-1" id="load-content">
	...
	<div class="row">
		...
		<div class="col-md-3 form-group">
			...
			<option value="PGM_CODE">Program Code</option>
			<option value="PGM_NAME">Program Name</option>
			</select>
			</div>
		</div>
        ...
        $totalResults = $main_app->sql_fetchcolumn("SELECT count(0) FROM {$page_table_name} WHERE {$add_sql}",$add_data);
            ...
            if($totalResults) {
                ...
                while ($row = $sql_exe->fetch()) {
					...
					echo "<td>".$main_app->strsafe_output($row['PGM_CATEGORY'])."</td>";
					if(isset($row['PGM_STATUS']) && $row['PGM_STATUS'] == "1") { echo "<td>".$main_app->strsafe_output("Enabled")."</td>"; } else { echo "<td>".$main_app->strsafe_output("Disabled")."</td>"; }
					echo "</tr>";
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#help-table tbody').on('click', 'tr', function () {
			...
			$('input[name="PGM_CODE"]').val(data[0]).trigger('change');
